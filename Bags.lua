...
function B:VendorWhites(delete)
	if self.SellFrame:IsShown() then return end
	...
	for bagID = 0, 4 do
		for slotID = 1, GetContainerNumSlots(bagID) do
			local _, itemCount, _, _, _, _, itemLink, _, _, itemID = GetContainerItemInfo(bagID, slotID)
			if itemLink and not (select(4, GetItemInfo(itemID)) == 0) and select(3, GetItemInfo(itemID)) == LE_ITEM_QUALITY_COMMON then
				local itemValue = select(11, GetItemInfo(itemID))
				totalValue = totalValue + (itemValue * itemCount)
				itemList[#itemList + 1] = {bagID, slotID}
			end
		end
	end

	if #itemList == 0 then
		E:Print(L["No white items to sell."])
	elseif not MerchantFrame:IsShown() then
		E.PopupDialogs.DELETE_GRAYS.Money = totalValue
		E:StaticPopup_Show("DELETE_GRAYS")
	else
		local info = self.SellFrame.Info
		info.delete = delete or false
		info.SellTimer = 0
		info.ProgressMax = #itemList
		info.ProgressTimer = (#itemList - 1) * info.SellInterval
		info.UpdateTimer = 0
		info.goldGained = 0
		info.itemsSold = 0
		...
		self.SellFrame.statusbar:SetValue(0)
		self.SellFrame.statusbar:SetMinMaxValues(0, #itemList)
		self.SellFrame.statusbar.ValueText:SetFormattedText("0 / %d", #itemList)

		self.SellFrame:Show()
	end
end

function B:VendorWhiteCheck()
	local itemCount, value = B:GetWhitesInfo()
...
function B:ContructContainerFrame(name, isBank)
	...
	if isBank then
		...
		--Vendor Whites
		f.vendorWhitesButton = CreateFrame("Button", nil, f.holderFrame)
		f.vendorWhitesButton:Size(16 + E.Border)
		f.vendorWhitesButton:SetTemplate()
		f.vendorWhitesButton:Point("BOTTOM", f.vendorGraysButton, "TOP", 0, 3)
		f.vendorWhitesButton:SetNormalTexture("Interface\\ICONS\\INV_Misc_Coin_01")
		f.vendorWhitesButton:GetNormalTexture():SetTexCoord(unpack(E.TexCoords))
		f.vendorWhitesButton:GetNormalTexture():SetInside()
		f.vendorWhitesButton:SetPushedTexture("Interface\\ICONS\\INV_Misc_Coin_01")
		f.vendorWhitesButton:GetPushedTexture():SetTexCoord(unpack(E.TexCoords))
		f.vendorWhitesButton:GetPushedTexture():SetInside()
		f.vendorWhitesButton:StyleButton(nil, true)
		f.vendorWhitesButton.ttText = L["Vendor / Delete Whites"]
		f.vendorWhitesButton:SetScript("OnEnter", B.Tooltip_Show)
		f.vendorWhitesButton:SetScript("OnLeave", GameTooltip_Hide)
		f.vendorWhitesButton:SetScript("OnClick", B.VendorWhiteCheck)
